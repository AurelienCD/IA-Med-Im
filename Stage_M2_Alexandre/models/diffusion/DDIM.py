...
class DDIM:
    ...
    def one_step_reverse_sampling(self, xt: torch.Tensor, t: torch.Tensor):
        ...
        alpha_bars_t = extract(self.alpha_bars, t, xt.shape)
        alpha_bars_t_prev = extract(self.alpha_bars, prev_t, xt.shape)
    ...
    def rec_reverse_sampling(self, img_batch: torch.Tensor):
        ...
        batch_image_reverse_step = [img_batch.cpu().numpy()]
        ...
        for t in tqdm(reversed(range(self.timestep)), desc="reverse process"):
            ...
            img_batch = self.one_step_reverse_sampling(img_batch, t_batch)
            batch_image_reverse_step.append(img_batch.cpu().numpy())
    ...
    def compute_loss(self, x0_batch, noise=None, type='l2'):
        ...
        if type == 'l2':
            ...
            return nn.functional.l1_loss(noise, predicted_noise)
        else:
